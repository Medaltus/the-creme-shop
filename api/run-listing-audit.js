...
- SIZE FORMAT IN TITLE — variation-family exception, added 2026-09-18 per Jaclyn: Size must always appear at the end of the title in parentheses, UNLESS this product is one of several SCENT-differentiated siblings sharing the same base product/formula. Check "Related SKUs in this catalog" below: if the sibling names share the same product name and differ only in a trailing scent/flavor descriptor (e.g. "...Hand Crème (Strawberry Milkshake)" vs "...Hand Crème (Peach Fizz)" vs "...Hand Crème (Mint Chip)"), this product is part of a scent-differentiated variation family. In that case scent — not size — is the real differentiator a shopper uses to choose between otherwise near-identical siblings (same or similar ingredients), so the SCENT NAME goes in parentheses at the end of the title instead of size. Size must still be present in the title when this applies, but placed in the flowing text before the parenthetical scent, not inside the parentheses and not omitted — e.g. "The Creme Shop x Pusheen Hand Creme 1 fl oz (Strawberry Milkshake)", NOT "...Hand Creme Strawberry Milkshake (1 fl oz)" (scent buried mid-title, size given the differentiator slot it doesn't deserve here) and NOT "...Hand Creme (1 fl oz)" with the scent dropped entirely. Liquid products (serums, shampoos, conditioners, masks, oils) still use "fl oz", non-liquid/powder products (hand creams, balms, powders) still use "oz" only, regardless of which placement applies. If the product is NOT part of a scent-differentiated family — siblings differ by size only, differ by some other attribute, or there are no siblings listed — the original rule applies unchanged: size stays in parentheses at the end, exactly as before. State in TITLE_NOTES which case applied (scent-differentiated family vs. standard) and why, so this reasoning is visible on review, not just the resulting rewrite.
